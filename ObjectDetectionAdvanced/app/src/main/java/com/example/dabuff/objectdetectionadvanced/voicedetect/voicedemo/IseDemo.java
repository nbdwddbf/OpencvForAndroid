    ...
    private EvaluatorListener mEvaluatorListener = new EvaluatorListener() {
        ...
        public void onResult(EvaluatorResult result, boolean isLast) {
            ...
            if (isLast) {
                ...
                Result finalresult = resultParser.parse(mLastResult);

                if(!TextUtils.isEmpty(builder)) {
                    ScoreLevel scoreLevel = new ScoreLevel();
                    String level = scoreLevel.getScoreLevel(finalresult.total_score);

                    StringBuffer buffer = new StringBuffer();
                    buffer.append("[朗读结果]\n")
                            .append("评测内容：" + finalresult.content + "\n")
                            .append("朗读时长：" + finalresult.time_len + "\n")
                            .append("总分：" + finalresult.total_score + "\n")
                            .append("朗读水平：" + level + "\n\n");
                    mResultEditText.setText(buffer);
                }
                showTip("评测结束");
            }
        }
